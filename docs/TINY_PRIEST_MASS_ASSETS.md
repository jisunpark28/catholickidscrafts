...
Tiny Priest 3D는 **스프라이트 몸 + 프로시저럴 팔/손 오버레이**로 제스처를 냅니다 (`script.js` — `gesturePoses`, 키 1–7, Mass sequence).

| 뷰 | 팔 포즈 | 이유 |
|----|---------|------|
| **front** | 기도 자세(손 모음) 또는 **팔을 몸에서 약간 떼고 손 보이게** | 정면 정지·미사 응답 UI |
| **back** | **양팔 자연스럽게 아래, 손이 옆에 보임** (신부 `priest_back.png` 참고) | 걷기·뒤돌아보기; 오버레이 손이 겹치지 않음 |

- PNG에 손을 **고정 기도 자세로만** 그리면 `pray` / `signCross` / `hold` 제스처와 **겹쳐 보일 수 있음** → back은 반드시 **손이 자유로운 neutral**.
- 코드: 아이 플레이어 연동 시 `shouldShowOverlayHands()`를 child 캐릭터에 **true**로 두어 팔 애니메이션 활성화 (현재 신부/수녀는 `false`).
- 최종 일러스트: 팔꿈치~손목 구간은 단순한 실루엣으로 — 오버레이 손 색은 스프라이트 팔레트에서 샘플링.

**AI 샘플 (방향용):** `assets/mass/characters/*_SAMPLE.png` — front/back 쌍 4장.
