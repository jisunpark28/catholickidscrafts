...
import { parseBulkWordLines } from "@/lib/play-words-parse";
import Link from "next/link";
import { useCallback, useMemo, useState } from "react";

export type PlayWordRow = {
  id: string;
  word: string;
  hint: string;
  sortOrder: number;
...
type Props = {
  initialItems: PlayWordRow[];
  config: PlayWordsAdminConfig;
};

export function PlayWordsManager({ initialItems, config }: Props) {
  const [items, setItems] = useState<PlayWordRow[]>(initialItems);
  ...
  const refreshFromServer = useCallback(async () => {
    ...
    if (!res.ok) return;
    const data = (await res.json()) as PlayWordRow[];
    setItems(data);
  }, [config.apiBase]);
  ...
  async function addOne(nextWord: string, nextHint: string) {
    ...
    setItems((prev) =>
      [...prev, data as PlayWordRow].sort((a, b) =>
  ...
  async function handleBulkAdd() {
    const parsed = parseBulkWordLines(bulkText);
    if (parsed.length === 0) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${config.apiBase}/bulk`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function patchWord(id: string, patch: Partial<Pick<PlayWordRow, "hint" | "published">>) {
    setError("");
              ...
              filtered.map((item) => (
                <tr key={item.id} className="group">
                  ...
                  <td className="border-b border-[var(--color-border)] p-3 text-right">
                    ...
                    <Link
                      href={`${config.editPathPrefix}/${item.id}/edit`}
